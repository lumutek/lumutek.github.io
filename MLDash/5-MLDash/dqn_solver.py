...
class DQNSolver:  
    ...
    def __init__(self, observation_space, action_space, alpha, epsilon_max, batch_size, buffer_size): 
        #action space = 4 (environment measurements)
        self.observationSpace = observation_space
        ...
        self.explorationRate = epsilon_max 
             
                         
        #acts as a memory buffer of a specific size; FIFO, can create low memory condition if too large 
        self.memory = PrioritizedBuffer(self.batchSize, self.bufferSize)   
        self.verbosity = 0
        ...
        self.model.add(Dense(24, input_shape=(self.observationSpace,), activation="relu"))  
        # fully interconnected input layer with 24 neurons
    ...
    def remember(self, state, action, reward, state_next, terminal):
        #adds this experience tuple to a replay memory buffer, which is used later to perform prioritized sampling of experiences to better train the neural network for Q-value approximation.       
    ...
    def act(self, state):  
        #if a random number, between 0 and 1, is less than the current exploration factor...
    ...
    def experience_replay(self, gamma, epsilon_decay, epsilon_min):
        ...
        if (len(self.memory.buffer) < self.bufferSize):
            return
        else:
            print("\n*********************\nExperience Replay\n*********************\n")
            # sample a batch from the buffer using prioritized sampling
            indices, priorities = self.memory.get_prioritized_indices(self.batchSize)
            batch = [self.memory.buffer[i] for i in indices]
            priorities = np.array(priorities)

            # This code block, and the following for-loop, iterate through the batch samples
            ...
            #td_errors stands for "temporal difference" errors, which are used as a measure of how surprising or unexpected a particular experience was for the learning agent compared to its predictions.
            td_errors = np.zeros(self.batchSize)
            ...
            for i, (state, action, reward, next_state, terminal, _) in enumerate(batch):
                ...
                self.model.fit(state, q_values, verbose=self.verbosity)
            ...
            self.explorationRate *= epsilon_decay
            self.explorationRate = max(epsilon_min, self.explorationRate)
