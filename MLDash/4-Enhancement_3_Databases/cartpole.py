...
class MLCartpole:        
    def __init__(self, session, a, g, emin, emax, edec, eps, bat, buf):
        self.solved = False
    ...
    def cartpole(self):  
        ...
        while self.run <= self.max_runs:   
            ...
            while self.step < self.max_steps:  
                ...
                if terminal:  
                    self.solved = scoreLogger.add_record(self.session, self.step, self.run, dqn_solver.exploration_rate, self.gamma, self.alpha, self.batch_size, self.buffer_size)  
                    ...
                    if self.solved == True:
                        self.total_steps = self.total_steps + self.step
                        scoreLogger.add_summary(self.session, self.total_steps, self.run, self.alpha, self.gamma, self.epsilon_min, self.epsilon_max, self.epsilon_decay, self.numEpisodes, self.batch_size, self.buffer_size)  

                        return self.solved
                    if (self.run % self.numEpisodes == 0):
